/* =========================================================
   UPDATE 3.1 — simulação contínua do mundo
   ========================================================= */
...
export function update(dt){
 state.t+=dt;
 // O dia corre continuamente: 300 segundos = 24 horas do mundo.
 state.clock=(state.clock+dt/300)%1;ensureProgression();save.progression.clock=state.clock;
 ...
 const world={playerX:p.x,playerY:p.y,isNight:night,maxX,worldHour:clockLabel(),event:state.worldEvent};for(const entity of state.entities)updateCreature(entity,dt,world);
 tickFishing(dt,isDown(' '));updateResources(dt);
 stepTimer+=dt;if(stepTimer>.55&&Math.abs(p.vx)>80){stepTimer=0;save.progression.stats.steps++;}
