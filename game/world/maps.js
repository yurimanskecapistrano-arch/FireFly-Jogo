...
import {spawnResources,nearestResource,gatherNearest,craft,buyUpgrade,progressSummary} from '../systems/progression.js';
import {RECIPES,UPGRADES,RESOURCE_TYPES} from '../data/progression-data.js';
...
function workshopDialog(){
  ...
  const buttons=[['FECHAR',closeDialog]];
  for(const r of RECIPES){
    const cost=Object.entries(r.cost).map(([id,n])=>`${RESOURCE_TYPES[id].icon}${n}`).join(' ');
    buttons.unshift([`${r.name} · ${cost}`,()=>{craft(r.id);closeDialog()}]);
  }
  dialog('Bancada de Tito',`Nível ${s.level} · XP ${s.xp}. Transforme recursos em equipamentos permanentes.`,buttons);
}
function upgradeDialog(){
  const buttons=[['FECHAR',closeDialog]];
  for(const u of UPGRADES) buttons.unshift([`${u.name} · ${u.cost}✦`,()=>{buyUpgrade(u.id);closeDialog()}]);
  dialog('Oficina da Vila','Use moedas para melhorar sua expedição. Upgrades são permanentes.',buttons);
}
...
export function buildMap(){
  ...
  if(state.map==='village'){
    ...
    addInteractable('workshop',790,505,100,'craft',workshopDialog,'Usar bancada');
    addInteractable('upgrade',875,505,100,'upgrade',upgradeDialog,'Melhorar equipamentos');
...
export function handleWorldInteraction(){
  if(nearestResource()) return gatherNearest();
  const interactable=activeInteractable();
  if(interactable){interactable.onInteract();return true;}
  return false;
}
